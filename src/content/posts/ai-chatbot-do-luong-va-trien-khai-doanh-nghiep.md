...
Chính ở chỗ này, những khuyến nghị từ các đội làm platform khá đáng để học. [OpenAI](https://developers.openai.com/api/docs/guides/agent-evals) khuyên nên bắt đầu bằng trace khi còn đang debug hành vi, vì lúc đó câu hỏi thật sự nằm ở hành vi nội bộ của bot: chọn tool, timing handoff, instruction compliance và safety policy. Ở tài liệu [trace grading](https://developers.openai.com/api/docs/guides/trace-grading), họ nói rất rõ rằng trace cần được chấm ở cấp quyết định, tool call và reasoning step, thay vì chỉ nhìn output cuối cùng.
